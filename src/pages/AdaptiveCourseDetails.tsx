...
const TokensTab: React.FC<{ course: any }> = ({ course }) => {
  ...
  const isClaude = course.gen_provider === 'claude';
  return (
    <div className="space-y-4">
      {isClaude && (
        <div className="rounded-lg border border-violet-500/30 bg-violet-500/10 px-4 py-3">
          <p className="text-[13px] font-medium text-violet-300">
            ✦ Generated by Claude Code — off-platform
          </p>
          <p className="mt-0.5 text-[12px] text-text-mute">
            Content was authored and verified by Claude (3-judge MCQ consensus + Judge0 for coding), not the
            platform's OpenAI generator. The tokens below are the Claude agent tokens spent generating &amp;
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <Card padding="sm">
          <p className="text-[12px] uppercase tracking-wide text-text-mute">
            {isClaude ? 'Claude tokens' : 'Total tokens'}
          </p>
        ...
        <Card padding="sm">
          <p className="text-[12px] uppercase tracking-wide text-text-mute">
            {isClaude ? 'Generator' : 'Generation jobs'}
          </p>
          <p className="mt-1 text-xl font-semibold text-text">
            {isClaude ? 'Claude' : courseJobs.length}
          </p>
        </Card>
      </div>

      {course.gen_total_tokens === 0 && !isClaude && (
